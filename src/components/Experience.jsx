...
const CURVE_TANGENT_AIRPLANE = 0.02;
...
We have a wide range of beverages!`,
  ...
  useFrame((_state, delta) => {
    ...
    const tangent = curve.getTangent(
      lerpedScrollOffset + CURVE_TANGENT_AIRPLANE
    );
  ...
  useLayoutEffect(() => {
    ...
    planeOutTl.current.from(airplane.current.position, {
      duration: 10,
      z: -250,
      y: 10,
    });
    ...
    () => (
      <>
        ...
        {textSections.map((textSection, index) => {
          return <TextSection {...textSection} key={index} />;
        })}
        ...
        {clouds.map((cloud, index) => (
          <Cloud sceneOpacity={sceneOpacity} {...cloud} index />
        ))}
      </>
    ),
    []
